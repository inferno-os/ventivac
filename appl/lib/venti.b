...
Root.unpack(d: array of byte): ref Root
{
	...
	r := ref blankroot;
	r.version = g16(d, 0);
	if(r.version != Rootversion && r.version != Rootversionvar){
...
Entry.pack(e: self ref Entry): array of byte
{
	...
	if(i != len d) {
		sys->werrstr(sys->sprint("bad length, have %d, want %d", i, len d));
		return nil;
	}
	return d;
}
...
Entry.unpack(d: array of byte): ref Entry
{
	...
	if((e.flags & Entryactive) == 0)
		return e;
	varblocks := e.flags&Entryvarblocks;
	if(!checksize(e.psize) || (varblocks && e.dsize != 0 || !varblocks && !checksize(e.dsize))){
